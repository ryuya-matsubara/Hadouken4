...
- 🤖 **CPU対戦**（新規）— 人間（プレイヤー1）がコンピュータ（プレイヤー2）と対戦。ブラウザ内だけで完結し、サーバー通信は不要です。
- 🎮 **ローカル対戦**（従来どおり）— 1台の端末で交互に操作する「パス＆プレイ」方式。
- 🌐 **オンライン対戦**— 2台のスマートフォン／ブラウザからインターネット経由で対戦。AWS のサーバーレス構成（API Gateway WebSocket + Lambda + DynamoDB）で動作します。

> CPU対戦・ローカル対戦はサーバー不要で動きます。オンライン対戦を使う場合のみ、後述の AWS バックエンドをデプロイして `online-config.js` に WebSocket URL を設定してください。

---

## 目次

- [あそび方（CPU対戦）](#あそび方cpu対戦)
...
サーバー不要。`index.html` を開くだけで遊べます。

1. `index.html` をブラウザで開く。
2. 「🤖 CPU対戦」を押す。
3. あなた（プレイヤー1）がキャラクターを選び、「キャラクターを決定」を押す。CPU（プレイヤー2）のキャラクターは自動で選ばれます。
4. 毎ターン、あなたがアクションを選んで「決定」を押すと、**CPUが自動的にアクションを決定**します。
5. 「⚔️Battle⚔️」でバトルシーンが再生され、以降は他モードと同じルールで進行します。
6. どちらかのHPが0になれば勝者を表示。「もう一度あそぶ」で再戦できます。

CPU はランダムではなく、**現在の HP・エネルギー・双方のキャラクター・プレイヤーの行動履歴**を踏まえ、
既存のゲームエンジンで各手をシミュレーションして期待値の高い行動を選びます（詳細は [アーキテクチャ](#アーキテクチャ) を参照）。
...
**CPU の思考ロジック**は `shared/cpuAI.js` に分離されています。CPU は独自にルールを再実装せず、
`shared/gameEngine.js` の `validateAction` / `resolveTurn` などを注入して各手をシミュレーションし、
合法手の中から期待値ベースでソフトマックス選択します。乱数は差し替え可能（DI）で、テストは決定的に検証できます。
UI（`index.html`）・ルール判定（`gameEngine.js`）・CPU思考（`cpuAI.js`）・対戦モード制御は互いに分離しています。
...
ルールエンジン、ルームの整合性（二重送信・同時更新など）、および CPU の思考ロジックのテストです。
...
期待結果：`# pass 49 / # fail 0`。テスト項目は以下を含みます。
...
- **CPU思考**（`shared/cpuAI.js`）：エネルギー不足の手を選ばない／相手エネルギー0でガードが無意味なら選ばない／
  合法手のみ返す／プレイヤー履歴が予測確率に反映される／使用不能なプレイヤー手を予測から除外／
  勝利手を高評価・即敗北手を低評価／必殺技を含めて選択／再戦時の履歴リセット／乱数DIによる決定性
...
- CPU 思考ロジック `shared/cpuAI.js`（ルール判定はエンジンを注入して再利用。乱数DIで決定的にテスト可能）。CPU対戦はブラウザ内で完結し、バックエンド通信を使いません。
