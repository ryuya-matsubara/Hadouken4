...
CPU はランダムではなく、**現在の HP・エネルギー・双方のキャラクター・ゲームルール**だけを踏まえ、
既存のゲームエンジンで各手をシミュレーションして評価します（プレイヤーの行動履歴は使いません）。
明確な悪手は選ばず、合理的な選択肢が複数あるときは確率的に分散して選びます（詳細は [アーキテクチャ](#アーキテクチャ) を参照）。
...
`shared/gameEngine.js` の `validateAction` / `resolveTurn` などを注入して各手をシミュレーションします。
流れは、(1) 合法手を抽出→(2) 明確に無意味な手を除外→(3) プレイヤーの合法手を**均等確率**とみなし（履歴は不使用）→
(4) 各手の**期待値**を計算→(5) 明確に劣る手を除外→(6) 期待値を **0〜1 に正規化**してからソフトマックス、というもの。
正規化を挟むことで、生スコアの絶対値ではなく手の相対的な優劣で確率が決まり、
**合理的な複数の手に確率が分散**します（唯一の明確な最適手は 100%、明確な悪手は 0%）。
乱数は差し替え可能（DI）で、テストは決定的に検証できます。
...
- **CPU思考**（`shared/cpuAI.js`）：エネルギー不足の手を選ばない／相手エネルギー0でガードが無意味なら選ばない／
  合法手のみ返す／プレイヤーの手は均等確率で予測（履歴不使用）／使用不能なプレイヤー手を予測から除外／
  勝利手は支配的で100%選択・即敗北手を低評価／必殺技を含めて選択／乱数DIによる決定性／
  **確率分散**（両者Energy>0でBlastが100%にならない・CPU Energy0でGuardが100%にならない・両者Energy0でCharge 100%）
